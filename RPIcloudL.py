...
def rising_activation_callback(channel):
    db = connect_db()
    cur = db.execute('SELECT light_name, mode FROM light ORDER BY pin_id DESC')
    l_list = cur.fetchall()
    cur = db.execute('SELECT config_status FROM config ORDER BY config_id DESC')
    config_status = cur.fetchall()
    from_email = query_db("SELECT email,email_pw FROM email WHERE access = 0")
    to_email = query_db("SELECT email FROM email WHERE access = 1")
    for row in l_list:
        if row[1] == "2":
            gpio_out_both(int(row[0]))
        elif row[1] == "3":
...
def rising_dark_callback(channel):
    ...
    for row in l_list:
        if row[1] == "1":
            gpio_out_both(int(row[0]))
    if dark:
        db.execute("UPDATE config SET config_status = '0' WHERE config_name = 'dark'")
    else:
        db.execute("UPDATE config SET config_status = '1' WHERE config_name = 'dark'")
    db.commit()
    db.close()
# 13端口下沿电压回调函数
...
# 11端口上沿电压中断检测
GPIO.add_event_detect(11,GPIO.BOTH,callback = rising_activation_callback,bouncetime = 200)
# 11端口下沿电压中断检测
# GPIO.add_event_detect(11,GPIO.FALLING,callback = falling_activation_callback,bouncetime = 200)
# 13端口上沿电压中断检测
GPIO.add_event_detect(13,GPIO.BOTH,callback = rising_dark_callback,bouncetime = 200)
# 13端口下沿电压中断检测
...
def switch():
    if not session.get("logged_in"):
        return redirect(url_for("login"))
    db = connect_db()
    cur = db.execute('SELECT light_name, status FROM light ORDER BY pin_id DESC')
    imglist = [dict(light=row[0], status=row[1]) for row in cur.fetchall()]
    db.close() 
    if request.method == 'POST':
        L_id = int(request.form['up'])
        gpio_out_both(L_id)

    #return imglist
    return render_template("switch.html",imglist = imglist)
